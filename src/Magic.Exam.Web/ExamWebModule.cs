...
public class ExamWebModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        ...
        if (!hostingEnvironment.IsDevelopment())
        {
            ...
            {
                serverBuilder.AddProductionEncryptionAndSigningCertificate("openiddict.pfx", "d20bfa9a-e1e2-49f8-b5cf-bc957fd6b559");
            });
        }
    }
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<ExamDomainSharedModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}Magic.Exam.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<ExamDomainModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}Magic.Exam.Domain"));
                options.FileSets.ReplaceEmbeddedByPhysical<ExamApplicationContractsModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}Magic.Exam.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<ExamApplicationModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}Magic.Exam.Application"));
